define(['utilities/window','gl-capability','three'],function(window) {
	...
	window.THREEx.DragPanControls	= function(object, domElement)
	{
		this._object	= object;
	...
	window.THREEx.DragPanControls.prototype.destroy	= function()
	{
	...
	window.THREEx.DragPanControls.prototype.update	= function(event)
	{
	...
	window.THREEx.DragPanControls.prototype._onMouseMove	= function(event)
	{
		this._mouseX	= ( event.clientX / window.innerWidth ) - 0.5;
		this._mouseY	= ( event.clientY / window.innerHeight) - 0.5;
	}

	window.THREEx.DragPanControls.prototype._onTouchStart	= function(event)
	{
	...
	window.THREEx.DragPanControls.prototype._onTouchMove	= function(event)
	{
		if( event.touches.length != 1 )	return;
